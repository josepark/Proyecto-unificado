...
export default function DatacenterForm() {
  ...
  const { datos: datacentersResp, cargando: cargandoLista } = useApi(() => inventarioApi.datacenters(), []);
  const lista = datacentersResp?.results ?? datacentersResp ?? [];
  const existente = editando ? lista.find((c) => String(c.id) === id) : null;
  ...
  if (editando && cargandoLista) return <p>Cargando centro de datos…</p>;
  if (editando && !cargandoLista && !existente) {
    return (
      <div className="card">
        <div className="cuerpo">
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <div className="cuerpo">
            <Fila>
              ...
              />
              <CampoSelect label="Tipo" opciones={OPC_TIPO} value={form.tipo} onChange={(e) => set('tipo', e.target.value)} />
            </Fila>
